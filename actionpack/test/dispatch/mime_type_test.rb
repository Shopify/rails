...
class MimeTypeTest < ActiveSupport::TestCase
  ...
  test "parse single" do
    Mime.const_get(:LOOKUP_BY_STRING).each_key do |mime_type|
...
class MimeTypeRegistryFreezeTest < ActiveSupport::TestCase
  ...
  test "after soft_freeze the registries are shareable" do
    Mime.soft_freeze

    assert_ractor_shareable Mime.const_get(:REGISTRY)
    assert_ractor_shareable Mime.const_get(:LOOKUP_BY_STRING)
    assert_ractor_shareable Mime.const_get(:LOOKUP_BY_EXTENSION)
  end

  test "registering after soft_freeze is deprecated, falls back to copy-on-write, and stays shareable" do
    Mime.soft_freeze

    assert_deprecated("after the application has been initialized", ActionDispatch.deprecator) do
      Mime::Type.register("text/x-ractor", :ractor_probe)
    end

    assert_equal Mime[:ractor_probe], Mime::Type.lookup("text/x-ractor")
    assert_includes Mime.symbols, :ractor_probe
  ...
  test "after soft_freeze a reference captured before the freeze no longer tracks unregister" do
    Mime::Type.register_alias "application/xhtml+xml", :foobar
  ...
  test "deprecated Mime::SET, Mime::LOOKUP and Mime::EXTENSION_LOOKUP proxies reflect registration after soft_freeze" do
    Mime.soft_freeze
    ...
    assert_deprecated("Mime::SET is deprecated", ActionDispatch.deprecator) do
      assert_includes Mime::SET.symbols, :ractor_probe
    end
    assert_deprecated("Mime::LOOKUP is deprecated", ActionDispatch.deprecator) do
      assert_equal Mime[:ractor_probe], Mime::LOOKUP["text/x-ractor"]
    end
    assert_deprecated("Mime::EXTENSION_LOOKUP is deprecated", ActionDispatch.deprecator) do
      assert_equal Mime[:ractor_probe], Mime::EXTENSION_LOOKUP["ractor_probe"]
    end
  end
end

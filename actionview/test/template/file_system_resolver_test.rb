...
class FileSystemResolverRactorTest < ActiveSupport::TestCase
  ...
  test "non-strict templates compile inside a non-main Ractor" do
    Dir.mktmpdir do |dir|
      ...
      # Record the notifier subscriptions as a shareable snapshot, so the
      # Ractor can build its own notifier: compile! and render instrument.
      ActiveSupport::Ractors.unshareable_proc_action = :raise
      ActiveSupport::Notifications.notifier = ActiveSupport::Notifications.notifier

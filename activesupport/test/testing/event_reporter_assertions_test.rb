  ...
  module Testing
    class EventReporterAssertionsTest < ActiveSupport::TestCase
      ...
      test "assert_event_reported fails when different event is reported" do
        ...
        assert_equal <<~MESSAGE.strip, e.message.strip
          Expected an event to be reported matching:
            name: user.created
            payload: {id: 123}
            tags: {}
          but none of the 1 reported events matched:
            another.event (payload: {id: 123, name: "John Doe"}, tags: {})
        MESSAGE
      end
      ...
      test "assert_no_event_reported fails when event is reported" do
        ...
        assert_equal <<~MESSAGE.strip, e.message.strip
          Expected no 'user.created' event to be reported, but found:
            user.created (payload: {id: 123, name: "John Doe"}, tags: {})
        MESSAGE
      end
    end
  end

    ...
    private
      ...
      class Callback # :nodoc:
        ...
        def initialize(name, filter, kind, options, chain_config)
          ...
          @name            = name
          @kind            = kind
          # Store the object_id (a shareable Integer) rather than the original
          # (possibly unshareable) filter proc, so the Callback can be made
          # Ractor-shareable. matches? compares against _filter.object_id.
        ...
        def freeze # :nodoc:
          ...
          # Convert filter/condition procs to shareable procs (honors
          # Ractors.unshareable_proc_action) before deep-freezing, so callbacks
          # can be made shareable at ractorize time rather than only at
          # registration.
          @filter = Ractors.make_shareable(try_shareable_proc(@filter))
          @if = make_conditionals_ractor_shareable(@if.map { |c| try_shareable_proc(c) })
          @unless = make_conditionals_ractor_shareable(@unless.map { |c| try_shareable_proc(c) })
          @compiled = nil

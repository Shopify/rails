      ...
      def handle_dependency
        ...
        when :destroy_async
          ...
          unless target.empty?
            ...
            primary_key_column = (association_class.query_constraints_list || Array(association_class.primary_key)).map(&:to_sym)

            ids = target.collect do |assoc|
              primary_key_column.map { |col| assoc.public_send(col) }
            end

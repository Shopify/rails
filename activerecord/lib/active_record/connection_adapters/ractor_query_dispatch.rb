...
module ActiveRecord
  module ConnectionAdapters
    ...
    # ractor_make_shareable(_, copy: true) on the calling side; the resulting
    ...
    module RactorQueryDispatch
      ...
      # ractor_make_shareable before being returned across the boundary.
      def select_all(model, arel, name = nil, binds = [], allow_retry: false)
        # +ractor_make_shareable(_, copy: true)+ deep-freezes the arel /
        ...
        shareable_arel  = ractor_make_shareable(arel,  copy: true)
        shareable_binds = ractor_make_shareable(binds, copy: true)
        model_name      = model.name
        query_name      = name.nil? ? nil : ractor_make_shareable(name, copy: true)
        ...
        Ractor::Dispatch.main.run do
          ...
          ractor_make_shareable(result)
        end
      end
    end

    ractor_make_shareable(RactorQueryDispatch)
  end
end

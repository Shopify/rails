...
module ActiveRecord
  module ConnectionAdapters
    class RactorConnectionProxy < AbstractAdapter # :nodoc:
      ...
      # boundaries. This proxy dispatches compile calls to the main ActiveSupport::Ractors.
      class VisitorProxy # :nodoc:
        INSTANCE = new.freeze
      ...
      class << self
        ...
        def shareable_copy(value)
          ...
          case value
          when Array
            ActiveSupport::Ractors.make_shareable(value.map { |v| shareable_copy(v) })
          when Hash
            ActiveSupport::Ractors.make_shareable(value.to_h { |k, v| [shareable_copy(k), shareable_copy(v)] })
          else
            if defined?(ActionText::Content) && value.is_a?(ActionText::Content)
              ActiveSupport::Ractors.make_shareable(value.to_html.to_s)
            elsif defined?(Nokogiri::XML::Node) && value.is_a?(Nokogiri::XML::Node)
              ActiveSupport::Ractors.make_shareable(value.to_html.to_s)
            elsif defined?(Nokogiri::XML::Document) && value.is_a?(Nokogiri::XML::Document)
              ActiveSupport::Ractors.make_shareable(value.to_html.to_s)
              ...
              ActiveSupport::Ractors.make_shareable(value.to_s)
            end
          end
        end

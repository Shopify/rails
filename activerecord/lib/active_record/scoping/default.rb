  ...
  module Scoping
    ...
    module Default
      ...
      module ClassMethods
        ...
        # Pre-computes the +default_scope_override+ flag at boot so the
        # lazy assignment inside +build_default_scope+ never fires from a
        # non-main Ractor. The lazy path writes the flag the first time
        # +build_default_scope+ runs; the writer is generated by
        # +class_attribute :default_scope_override+ and goes through
        # +ActiveSupport::ClassAttribute.redefine+, which calls
        # +instance_variable_set+ on the singleton class. From a non-main
        # Ractor that raises +Ractor::IsolationError+. The value is pure
        # boot-time metadata (a method-owner check that asks whether the
        # AR descendant has overridden +self.default_scope+) and the
        # subsequent lazy compute is guarded by +nil?+, so warming the
        # value once on the main side is idempotent and equivalent to the
        # lazy compute. Mirrors the per-class warmers
        # (+make_callback_chains_shareable!+,
        # +make_normalized_attributes_shareable!+, etc.) wired into
        # +Rails.application#ractorize!+.
        def make_default_scope_override_shareable! # :nodoc:

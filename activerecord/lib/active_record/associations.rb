  ...
  module Associations # :nodoc:
    ...
    private
      ...
      module ClassMethods
        ...
        #   the foreign key match, while only the foreign key columns are written when assigning the association.
        #   The foreign key is derived by convention when +:foreign_key+ is omitted. Symbols and strings match
        #   same-named columns on both tables. A Hash maps a column on this model to one on the associated model,
        #   for example <tt>{ tenant_id: :account_tenant_id }</tt>. Hash mappings require +:foreign_key+.
        #   Query constraint columns must be distinct from the foreign key columns.
        # [+:index_errors+]

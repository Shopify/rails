...
class SchemaTest < ActiveRecord::PostgreSQLTestCase
  ...
  def test_columns_for_zero_column_table
    @connection.execute 'CREATE TABLE "pg_zero_col" ()'
    assert_equal [], @connection.columns("pg_zero_col")
  ensure
    @connection.drop_table :pg_zero_col, if_exists: true
  end

  def test_columns_raises_for_missing_table
    assert_raises(ActiveRecord::StatementInvalid) do
      @connection.columns("pg_does_not_exist")
    end
  end

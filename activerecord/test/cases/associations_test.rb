...
class AssociationsTest < ActiveRecord::TestCase
  ...
  def test_query_constraints_that_dont_include_the_primary_key_raise_with_a_single_column
    blog_post_class = Class.new(ActiveRecord::Base) do
      self.table_name = "sharded_blog_posts"
      def self.name = "Sharded::BlogPost"
      def self.to_s = name

      query_constraints :title
      has_many :comments_without_single_column_query_constraints,
        primary_key: [:blog_id, :id],
        class_name: "Sharded::Comment"
    end
    blog_post = blog_post_class.new
  ...
  def test_query_constraints_that_dont_include_the_primary_key_raise_with_multiple_columns
    blog_post_class = Class.new(ActiveRecord::Base) do
      ...
      query_constraints :title, :revision
      has_many :comments_without_multiple_column_query_constraints,
        primary_key: [:blog_id, :id],
        class_name: "Sharded::Comment"
    end
    blog_post = blog_post_class.new
